...
exports.goals = (0, pg_core_1.pgTable)('goals', {
    ...
    user_id: (0, pg_core_1.integer)("user_id").references(() => users_1.users.id, { onDelete: 'cascade' }).notNull(),
    period: (0, pg_core_1.text)({ enum: ['daily', 'weekly', 'monthly', 'yearly'] }).notNull(),
    category: (0, pg_core_1.text)({ enum: ['personal', 'professional'] }).notNull(),
    title: (0, pg_core_1.text)().notNull(),
    description: (0, pg_core_1.text)(),
    status: (0, pg_core_1.text)({ enum: ['not_started', 'in_progress', 'completed', 'abandoned'] }).notNull(),
    priority: (0, pg_core_1.text)({ enum: ['high', 'medium', 'low'] }).notNull(),
